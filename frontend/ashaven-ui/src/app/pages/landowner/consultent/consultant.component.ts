...
@Component({
  selector: 'app-consultant-slider',
  standalone: true,
  imports: [CommonModule, RouterModule],
  templateUrl: './consultant.component.html',
  styleUrls: ['./consultant.component.css'],
})
export class ConsultantSliderComponent implements OnInit, OnDestroy {
  slides: Slide[] = [];
  ...
  loadProjects() {
    this.projectService.getConsultant().subscribe({
      next: (projects: Consultant[]) => {
        // keep only non-interior items (0). Coerce to number to be safe.
        const nonInterior = (projects ?? []).filter(
          (p) => Number(p.isInterior) === 0
        );
